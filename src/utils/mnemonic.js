...
const walletClientCurrent = new WalletClientCurrent('https://rpc.mainnet.aptos.fernlabs.xyz/v1', 'dummy')
...
export async function importWallet(code) {
  ...
  const account5 = await walletClientCurrent.importWallet(code);

  return [...account1.accounts, ...account2.accounts, ...account3.accounts, ...account5.accounts];
}
